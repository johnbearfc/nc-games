...
const ReviewComments = ({ review_id, commentCount }) => {
    ...
    return (
        <CommentsWrapper>
            <PostComment review_id={review_id} setComments={setComments} setCommentCountChange={setCommentCountChange}/>
            <h3>Comments ({commentCount + commentCountChange})</h3>
            <ul>
                {comments.map(comment => {
                    return (
                        <li key={comment.comment_id}>
                            <SingleComment review_id={review_id} comment={comment} setComments={setComments} setCommentCountChange={setCommentCountChange}/>
                        </li>
                    )
                })}
            </ul>
            {(commentCount + commentCountChange) > 10 && <button onClick={() => setCommentsExtended(!commentsExtended)}>
